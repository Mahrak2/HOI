...
class Manipulator_Control:
    def __init__(self) -> None:
        ...
        self.tasks = [
                    ...
                    Configuration3D("End-Effector Configuration",np.array([ 0.250, -0.050 , -0.30, 0 ]),-1)]
    ...
    def controller(self,event=None):
        if self.robot_position is None or self.q is None:
            return
        ...
        # print(f'error:{np.linalg.norm(current_position-self.goal_position[:3]):.3f}')
        # if np.linalg.norm(current_position-self.goal_position[:3])<0.001:
        #     dq = np.zeros((dof,1))
        #     self.__sent_velocity__(dq)
        #     self.goal_position = None
    ...
    def goal_callback(self, msg:PoseStamped):
        ...
        self.goal_position = np.array([x, y, z , 0])
        # self.tasks[-1].setDesired(np.array([x, y, z , 0]))
        self.last_time = rospy.Time.now()
        self.trajectory = []
        self.trajectory_limit = []
